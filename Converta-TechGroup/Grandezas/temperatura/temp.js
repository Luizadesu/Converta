...
function resultado() {
    var x = select.value;
    if (Number(x) == 1) //celsius selecionado
    {
        r1.innerHTML = `${valor.value} ºC em Farenheit equivale a: ${(valor.value * 9 / 5) + 32} ºF`
        r2.innerHTML = `${valor.value} ºC em Kelvin equivale a: ${Number(valor.value) + 273.15} K`
    }
    else if (Number(x) == 2) // farenheit selecionado
    {
        r1.innerHTML = `${valor.value} ºF em Celsius equivale a: ${Number(valor.value - 32) * 5 / 9} ºC`
        r2.innerHTML = `${valor.value} ºF em Kelvin equivale a: ${Number(valor.value) + 273.15} K`
    }
    else //kelvin selecionado
    {
        r1.innerHTML = `${valor.value} K em Farenheit equivale a: ${Number(valor.value) - 273.15} ºF`
        r2.innerHTML = `${valor.value} K em Celsius equivale a: ${Number(valor.value - 273, 15) * (9 / 5) + 32} ºC`
    }
