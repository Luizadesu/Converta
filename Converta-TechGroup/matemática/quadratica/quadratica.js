...
function resultado()
{
    ...
    if (delta > 0)
    {
    var b1 = (- b + Math.sqrt(delta)) / 2 * a
    var b2 = (- b - Math.sqrt(delta)) / 2 * a
    r1.innerHTML = `Existem duas raízes possíveis: ${b1} e ${b2}`
    }
    if (delta > 0)
    {
    var b1 = (- b + Math.sqrt(delta)) / 2 * a
    r1.innerHTML = `Existe apenas uma raíz: ${b1}`
    }
    else
    {
    r1.innerHTML = `Não existem raízes`
    }
